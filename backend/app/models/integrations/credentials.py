...
from app.core.security import decrypt_credential, encrypt_credential, EncryptionService
...
def _safe_encrypt(value: Any) -> Any:
    if value is None or not isinstance(value, str) or value == "":
        return value
    if EncryptionService.is_sealed(value):
        return value
...
def seal_amocrm_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    return EncryptionService.seal_mapping(dict(config), _SENSITIVE_AMOCRM_FIELDS)
...
def seal_bitrix_config(config: dict[str, Any]) -> dict[str, Any]:
    ...
    return EncryptionService.seal_mapping(dict(config), _SENSITIVE_BITRIX_FIELDS)
